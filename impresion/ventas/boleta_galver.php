...
//if (is_writable("boleta.xml")) 
//{
$itemsPagina=100;
$file = fopen("boleta.xml", "w");
...
{
	$fp = fopen("boleta.xml", "r"); 
	$data = fread ($fp, filesize("boleta.xml")); 
	fclose($fp); 
	$zp = gzopen("boleta.xml.gz", "w9"); 
	gzwrite($zp, $data); 
	gzclose($zp);
	echo "<script language='javascript'> self.location.href='boleta.xml.gz';  </script>"; 	
}
