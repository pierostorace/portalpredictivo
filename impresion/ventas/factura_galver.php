...
//if (is_writable("factura.xml")) 
//{
$itemsPagina=100;
$file = fopen("factura.xml", "w");
...
$sql ="SELECT 	Concat(co.sgo_vch_serie,'-',co.sgo_vch_numero) as Comprobante,
			...
			and sgo_vch_dpto = (select sgo_vch_dpto from tbl_sgo_ubigeo where sgo_int_ubigeo = dir.sgo_int_ubigeo) and sgo_vch_dist = '00'   and sgo_vch_pais = '139')))) as Direccion,
			...
			case when co.sgo_int_categoriacomprobante = 2 then
...
fwrite($file,"<Cliente>");
fwrite($file,"<RazonSocial><![CDATA[".$cliente."]]></RazonSocial>");
fwrite($file,"<Ruc>".$ruc."</Ruc>");
fwrite($file,"<Direccion><![CDATA[".utf8_decode(str_replace("Ñ","&#209;",$direccion))."]]></Direccion>");
...
{
	$fp = fopen("factura.xml", "r"); 
	$data = fread ($fp, filesize("factura.xml")); 
	fclose($fp); 
	$zp = gzopen("factura.xml.gz", "w9"); 
	gzwrite($zp, $data); 
	gzclose($zp);
	echo "<script language='javascript'> self.location.href='factura.xml.gz';  </script>"; 	
}
