...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // 'cols' => array(<colname>=>array('justification'=>'left','width'=>100,'link'=>linkDataName),<colname>=>....)
...
function ezText($text,$size=0,$options=array(),$test=0){
  ...
  // 'width'=> ancho del label donde se escribirá el texto.
  // 'top'=> numero de lineas que debe subir.
  // 'left'=> number, gap to leave from the left margin
  // 'right'=> number, gap to leave from the right margin
  // 'atop'=> numero, absoluto top posicion (sobreescribe 'top')
  ...
  if (is_array($options) && isset($options['leading'])) { ## use leading instead of spacing
	...
	} else if (is_array($options) && isset($options['top'])) {
    $height = $this->getFontHeight($size) * ($options['top'] - 1);
	} else {
  ...
  foreach ($lines as $line){
    ...
    while (strlen($line) || $start){
      ...
      $line=$this->addTextWrap($left,($this->y-$atop),($width==0?$right-$left:$width),$size,$line,$just,0,$test);
    }
  }
